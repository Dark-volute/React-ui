...
const Input: React.FunctionComponent<IProps> = props => {
  ...
  const c2 = {
    errorRight: errorPosition === 'right',
    errorBottom: errorPosition === 'bottom',
  };
  return (
    <div
      className={sc('wrapper', c1, {
        'wrapper-big': size === 'big',
    ...
    >
      ...
      <div className={sc('inputAndError', c2)}>
        <div
          className={sc('container', { 'container-hasAddon': before || after })}
        >
          {before && <span className={sc('before')}>{before}</span>}
          <input
            className={classNames(sc('', { hasError: error }), className)}
            style={{ width, ...style }}
            {...restProps}
            onKeyDown={onKeyDown}
          />
