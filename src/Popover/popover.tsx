...
const sc = createScopedClasses('popover');
...
class Popover extends PureComponent<PopoverProps, PopoverState> {
  static defaultProps = {
    trigger: 'click',
    block: false,
    position: 'top',
  ...
  private contentHandlers: Partial<ITriggers> | null =
  ...
  get content() {
    const content = (
      <div
        ref={this.refContent}
        className={sc('content', `content-${this.props.position}`)}
        {...this.contentHandlers}
      >
        {typeof this.props.content === 'string' ? (
          <span className={sc('content-stringWrapper')}>
            {this.props.content}
          </span>
        ) : (
          this.props.content
  ...
  private positionContent = () => {
    ...
    switch (this.props.position) {
      case 'top':
        content!.style.top =
          top - containerTop - contentHeight - distance + 'px';
        content!.style.left =
          left - containerLeft + width / 2 - contentWidth / 2 + 'px';
        break;
      case 'bottom':
        content!.style.top = top - containerTop + height + distance + 'px';
        content!.style.left =
          left - containerLeft + width / 2 - contentWidth / 2 + 'px';
        break;
      case 'left':
        content!.style.top =
          top - containerTop + height / 2 - contentHeight / 2 + 'px';
        content!.style.left =
          left - containerLeft - contentWidth - distance + 'px';
        break;
      case 'right':
        content!.style.top =
          top - containerTop + height / 2 - contentHeight / 2 + 'px';
        content!.style.left = left - containerLeft + width + distance + 'px';
        break;
      case 'topLeft':
        content!.style.top =
          top - containerTop - contentHeight - distance + 'px';
      ...
      case 'topRight':
        content!.style.top =
          top - containerTop - contentHeight - distance + 'px';
        content!.style.left =
          left - containerLeft + width - contentWidth + 'px';
        break;
      case 'bottomLeft':
        content!.style.top = top - containerTop + height + distance + 'px';
        content!.style.left = left - containerLeft + 'px';
  ...
  componentDidUpdate(
    prevProps: Readonly<PopoverProps>,
    prevState: Readonly<PopoverState>,
    snapshot?: any,
  ): void {
  ...
  get container() {
    return (
      (this.props.container && this.props.container.current) || document.body
    );
  }
  ...
  wrapInDiv(inner: ReactChild) {
    const { props } = this;
    return <div className={sc('', { block: props.block })}>{inner}</div>;
  }
  wrapInClickOutside(inner: ReactChild, handlers: Partial<ITriggers>) {
    ...
    return (
      <ClickOutside
        className={sc('', { block: props.block })}
  ...
  renderComponent(open: boolean, handlers: ITriggers) {
    ...
    return this.props.trigger === 'click'
      ? this.wrapInClickOutside(inner, handlers)
      : this.wrapInDiv(inner);
  }
}
